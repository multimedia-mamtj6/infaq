...
This folder contains the Google Apps Script (bound to a Google Sheet) that acts as the CMS backend for the Infaq site. It reads donation data from sheet tabs, computes derived stats, and publishes three JSON files to the `infaq` GitHub repo, which the static site (`script.js`) fetches at runtime.
...
2. `publishJsonToGithub()` collects data from several sheet tabs, builds three JSON payloads, and pushes each to GitHub via the Contents API (`uploadToGithub`).
